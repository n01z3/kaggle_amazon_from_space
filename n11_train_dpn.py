...
from n06_pytorch_utils import augment2
import cv2
from n05_train_torchiter import fbeta_score, logloss_score, f2beta
...
def check_iters(ags):
    ...
    for j, trn_data in enumerate(trn, 0):
        ...
        for i in range(2):
            plt.subplot(2, 5, 6 + i)
            plt.imshow(np.uint8(np.transpose(inputs[i], (1, 2, 0)) / 0.0167 + 128))
        break
...
def get_mod(ags):
    ...
    if ags.sfrom == 'i' or len(glob.glob(f'weights/{ags.versn}/{ags.nfold}/*json')) == 0:
        ...
        sym, arg_params, aux_params = mx.model.load_checkpoint(
            f'weights/imagenet/dpn/{ags.model}', 0)

        fea_symbol = sym.get_internals()["flatten_output"]
...
def pytorch_data_iter(ags):
    ...
    trn_dataset = CSVDataset(f'../../_data/fold{ags.nfold}/train.csv', transform=augment2)
...
def add_fit_args(train):
    ...
    train.add_argument('--versn', default='dpn92_1', type=str, help='version of net')
    train.add_argument('--begin', default=-1, type=int, help='start epoch')
    train.add_argument('--sfrom', default='-1', type=str, help='start epoch')

    train.add_argument('--model', default='dpn92', type=str, help='model name')
    train.add_argument('--isize', default=256, type=int, help='input image size')

    train.add_argument('--batch', default=88, type=int, help='the batch size')
    ...
    train.add_argument('--check', default=160, type=int, help='period of check in iteration')
    train.add_argument('--lrate', default=0.01, type=float, help='start learning rate')
